...
The model here says that a stem:[tt "general_approval"] has a stem:[tt "status"]
which is of type stem:[tt "approval_status"] (whatever that is), and
stem:[tt "approved_items"] (which is a SET of (unnamed) ENTITY). An instantiable
SUBTYPE would have to replace stem:[tt "GENERIC_ENTITY"] by a named entity,
say stem:[tt "plan"] (whatever that might be), to have a set of
approved stem:[tt "plan"]s.
...
Within SCHEMA stem:[tt "first"], entity stem:[tt "original"] does not know it is
a SUPERTYPE as stem:[tt "first"] knows nothing about the stem:[tt "second"]
SCHEMA.
